 ...
 */
export async function copyFileToClipboardMobile(src: string, imageId?: string): Promise<boolean> {
    if (!src) return false;
    ...
    try {
        ...
        if (src.startsWith('data:image')) {
            ...
            // but usually incoming src from NoteFileService on mobile will be a persistent file URI.
